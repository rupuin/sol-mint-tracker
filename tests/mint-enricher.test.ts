...
function token(overrides: Partial<Token> = {}): Token {
  return { mint: "test-mint", name: "Test Token", symbol: "TEST", ...overrides };
}
...
describe("MintEnricher", () => {
  ...
  beforeEach(() => {
    fetcher = mockTokenFetcher(token({ mint: "abc", name: "Test", symbol: "TST" }));
  ...
  describe("enrichment", () => {
    ...
    it("does not emit when token fetch returns null", async () => {
      ...
      await vi.waitFor(() => expect(fetcher.fetchBySignature).toHaveBeenCalled());
  ...
  describe("error handling", () => {
    it("emits enrichment error when fetch fails", async () => {
      fetcher.fetchBySignature = vi.fn().mockRejectedValue(new Error("API error"));
